...
| ViewModel | AndroidX Lifecycle 2.8 |
| Testing | JUnit 4 + MockK 1.13 |
...
- **Instant QR sharing** — generates a local HTTP server and QR code in seconds
- **No receiver app needed** — works with any browser on any device
- **Multi-file support** — share multiple files in one session
- **Token-secured sessions** — 16-byte random token + 15-minute TTL per session
- **Foreground service** — server stays alive with a persistent notification
- **Range request support** — browsers can resume interrupted downloads
- **Max 5 concurrent connections** — bounded thread pool prevents abuse
- **Offline HTML download page** — no external CDN, works fully on LAN
- **File type detection** — badges and icons for audio, video, PDF, archive, image, text
- **Download counter** — see how many times each session's files were downloaded
...
> **Note:** A physical device on a Wi-Fi network is strongly recommended. The file server binds to the local LAN IP — emulators use a NAT address that cannot be reached by other devices.
...
1. User taps **Select Files** and picks one or more files
2. App starts a `ForegroundService` that launches a NanoHTTPD HTTP server on a random port
3. Server generates a session token and a local URL: `http://<device-ip>:<port>/?token=<token>`
4. App generates a QR code from the URL and displays it
5. Any device on the same LAN scans the QR or opens the URL in a browser
6. The browser receives a styled HTML download page listing all files
7. Each file streams directly from the Android content resolver (never loaded fully into memory)
8. Session expires after 15 minutes or when the user taps **Stop Sharing**
...
```
app/src/main/kotlin/com/example/qlfs/
├── MainActivity.kt               # Compose host, permission handling, screen routing
├── QLFSApplication.kt            # Hilt application class
├── di/
│   └── AppModule.kt              # Hilt DI bindings (NetworkManager singleton)
├── model/
│   └── SharedFile.kt             # Parcelable file metadata model
├── network/
│   └── NetworkManager.kt         # Local IPv4 detection (Wi-Fi + hotspot)
├── qr/
│   └── QrGenerator.kt            # ZXing QR bitmap generator
├── server/
│   ├── FileServer.kt             # NanoHTTPD server (/, /download, /thumbnail)
...
├── ui/
│   ├── ShareScreen.kt            # File picker + idle screen
│   ├── SharingActiveScreen.kt    # QR code, URL, countdown, stop button
│   ├── AboutScreen.kt            # App info + GitHub link
│   └── ShareViewModel.kt         # State machine: Idle → FilesSelected → SharingActive
└── util/
    ├── FileSizeFormatter.kt      # Human-readable file sizes
    └── TokenStore.kt             # Secure random token generator
```
...
| Permission | Reason |
|-----------|--------|
| `INTERNET` | Run embedded HTTP server |
| `ACCESS_WIFI_STATE` / `ACCESS_NETWORK_STATE` | Detect local IP address |
| `FOREGROUND_SERVICE` / `FOREGROUND_SERVICE_DATA_SYNC` | Keep server alive |
| `POST_NOTIFICATIONS` | Show persistent sharing notification (Android 13+) |
| `READ_EXTERNAL_STORAGE` | File access on Android 12 and below |
